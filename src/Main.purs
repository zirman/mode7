...
startGame cnv =
  ...
  getFile (f wnd ctx) "./file.obj" where
    ...
    f wnd ctx str = requestAnimationFrame (gameLoop wnd cnv ctx (loadWaveFront str)) wnd *> pure unit
...
drawWaveFront ctx drawMat eyeMat (WaveFront vs vts vns fs) = Traversable.traverse_ drawFace fs where
  drawFace face =
    let eyePoly4 = toEyeCoordinates eyeMat (Array.catMaybes (map (\(Point3 vi _ti _ni) -> Array.index vs vi) face))
    in if isClipped eyePoly4
       then pure unit
       else let viewPoly2 = toDrawCoordinates drawMat eyePoly4
            in if isPolygonClockwise viewPoly2
               then pure unit
               else drawPolygon ctx viewPoly2

getFile :: (String -> Effect Unit) -> String -> Effect Unit
getFile eff pth =
  XMLHttpRequest.xmlHttpRequest ResponseType.string >>= \rq ->
  let f e = XMLHttpRequest.response rq >>= \x -> Maybe.fromMaybe (pure unit) (eff <$> x)
  in EventTarget.eventListener f >>= \ltr ->
